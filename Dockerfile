...
RUN apt-get install -y python3.11-dev && apt clean -y && \
    uv pip install .[core,dev,grain,gcp,vertexai_tensorboard] && uv cache clean
COPY . .
...
COPY --from=apache/beam_python3.11_sdk:2.64.0 /opt/apache/beam /opt/apache/beam
